...
set -euo pipefail
#############################################################
# Train DiT (text -> image).
#
# Usage:
#   bash scripts/train.sh path/to/config.py
#   NUM_GPUS=4 bash scripts/train.sh path/to/config.py
#   bash scripts/train.sh path/to/config.py --resume work_dirs/.../latest.pth
#############################################################
...
if [[ $# -lt 1 ]]; then
    echo -e "${RED}Usage:${NC} bash scripts/train.sh <config.py> [extra args]"
    exit 1
fi
...
if [[ ${NUM_GPUS} -gt 1 ]]; then
    "${ACCELERATE}" launch --multi_gpu --num_processes "${NUM_GPUS}" --mixed_precision "${MIXED_PRECISION}" \
        tools/train.py "${CONFIG_FILE}" --work-dir "${WORK_DIR}" "$@"
else
