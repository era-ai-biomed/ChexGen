...
from torch.nn import Module, Linear, init, LayerNorm
...
from radiffuser.models.block import PixArtMSBlock
from radiffuser.models.dit import DiT
...
class ControlT2IDiT(Module):
    ...
    def __init__(self, base_model: DiT, copy_blocks_num: int = 13) -> None:
        super().__init__()
    ...
    def forward(self, x, timestep, y, mask=None, c=None, **kwargs):
    ...
    def forward_with_dpmsolver(self, x, t, y, c, **kwargs):
        model_out = self.forward(x, t, y, c=c, **kwargs)
        return model_out.chunk(2, dim=1)[0]


    def forward_with_cfg(self, x, t, y, cfg_scale, c, mask, **kwargs):
        half = x[: len(x) // 2]
        combined = torch.cat([half, half], dim=0)
        return self.forward(combined, t, y, mask, c=c, **kwargs)
