    ...
    if rank == 0:
        ...
        if args.output_prediction:
            with open(args.output_prediction, 'w') as f:
                idx = 0  # 初始化索引
                for batch_data in data_loader:  # 对 data_loader 的每个批次进行循环
                    batch_size = len(batch_data['img'])  # 假设 'img' 是图像数据键，用于确定批大小
                    for j in range(batch_size):  # 遍历每个批次中的样本
                        filename = data_loader.dataset.data_infos[idx]['img_info']['filename']
                        f.write(filename)
                        prediction = outputs[idx]
                        for k in range(prediction.shape[0]):  # 遍历当前样本的预测值
                            f.write(' ' + str(np.around(prediction[k], 8)))
                        f.write('\n')
                        idx += 1  # 索引递增
