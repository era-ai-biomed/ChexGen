...
from radiffuser.datasets.dataloader import MultiEpochsDataLoader
...
def main(cfg):
    ...
    # Find the most recent checkpoint file in work_dir
    ...
    if cfg.multi_epoch_loader:
        loader = MultiEpochsDataLoader(dataset=dataset, **cfg.get("dataloader"))
    else:
    ...
    for epoch in range(start_epoch + 1, epoches + 1):
        ...
        with accelerator.accumulate(model):
            for x, (y, y_mask) in loader:
                ...
                loss_dict = diffusion.training_losses(model, x, t, dict(y=masked_y, mask=y_mask))
...
def parse_args():
    ...
    parser.add_argument('--out_file', type=str, default='qa.json', help='output file')
    ...
    parser.add_argument('--multi-epoch-loader', action='store_true', default=False, help='use multi-epoch data loader')
...
def merge_args(cfg, args):
    ...
    cfg.multi_epoch_loader = args.multi_epoch_loader
    return cfg
